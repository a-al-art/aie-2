...
def report(
    ...
    title: str = typer.Option("EDA Report", help="Заголовок отчёта в report.md"),
    min_missing_share: float = typer.Option(0.1, help="Порог доли пропусков (от 0.0 до 1.0). Колонки с большей долей считаются проблемными."
    ),
) -> None:
    """
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        if not problem_cols.empty:
            ...
            f.write(", ".join(problem_cols.index.tolist()) + "\n\n")
        else:
            f.write("Проблемных колонок не обнаружено.\n\n")
