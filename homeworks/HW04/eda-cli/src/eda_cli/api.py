...
# ---------- /quality-flags: СОБСТВЕННЫЙ ЭНДПОИНТ ----------

@app.post(
    "/quality-flags",
    tags=["quality"],
    summary="Возвращает все флаги качества, включая из HW03",
)
async def quality_flags(file: UploadFile = File(...)) -> dict:
    """
    Эндпоинт, возвращающий все флаги качества (включая has_suspicious_id_duplicates,
    has_constant_columns и другие из HW03) без решения 'ok_for_model'.
    """
    if file.content_type not in ("text/csv", "application/vnd.ms-excel", "application/octet-stream"):
    ...
    # Преобразуем numpy.bool_ в bool, чтобы Pydantic мог сериализовать
    return {"flags": {k: bool(v) for k, v in flags.items()}}